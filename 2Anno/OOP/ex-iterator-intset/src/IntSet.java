...
public class IntSet {
	 ...
	 */
	/**
	 * ABSTRACTION FUNCTION: the set is represented by the items in
	 * this.elements
	 *
	...
	/**
	 * @param: elts.
	 *             REQUIRE be not null. EFFECT initialize this to a new set,
	 *             which contains each element of elts; duplicated elements are
	 *             not considered.
	 */
	...
	/**
	 * Copy constructor. EFFECT initialize this to a new set that contains all
	 * and only the elements of s.
	 * 
	 * @param s:
	 *            a set to be duplicated. REQUIRE not null.
	 */
	...
	/**
	 * @return: the index of x if it is present in this ; return -1 if not
	 *          present
	 */
	...
	/**
	 * ============================================= INNER CLASS for generic
	 * iterator =============================================
	 */
	/**
	 * MISSION is to provide an iterator over the elements of an IntSet. Once
	 * the iterator is created, if the original set changes the iterator
	 * continues to work on the original copy.
	 */
	private class IntSetIterator implements Iterator<Integer> {
		/**
		 * INVARIANT elements contains a copy of the elements of the IntSet when
		 * this iterator is created. current is the integer i such that
		 * elements[i] is the first of the elements not yet returned.
		 * current==elements.size if we explored all of them.
		 */
		private int current;
		...
		/**
		 * @param s
		 *            an IntSet REQUIRE not null Initialize the iterator with
		 *            the size of the vector and with current index=0, and store
	...
	public static void main(String[] args) {
		int[] elts = {7,1,2,3,9,8,4,5};
		IntSet is1 = new IntSet(elts);
		Iterator<Integer> it = is1.iterator();
		//IntSetIterator it = (IntSetIterator) is1.iterator();
		while(it.hasNext()){
			Integer i = it.next();
