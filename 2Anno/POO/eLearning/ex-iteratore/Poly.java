...
//-----------------------------	
	private class TermIterator implements Iterator<Poly> {
		private int current;
		final private Vector<Poly> terms;
		
		@SuppressWarnings
		("unchecked")
		TermIterator(Poly p){
		Objects.requireNonNull(p);
		this.terms = (Vector<Poly>) p.terms.clone();
		this.current = 0;
		}
		@Override
		...
		public Poly next(){	 
				Poly res = this.terms.get((int)this.current);
				this.current++;
